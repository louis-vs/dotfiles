...
- **neovim** — LazyVim. `lua/config/extras.lua` selects LazyVim extras, `lua/plugins/*.lua` overrides them (one file per concern; `disable.lua` turns plugins off). `lazy-lock.json` is tracked, which is what pins the plugin set (the spec itself uses `version = false`, i.e. always latest commit). lazy.nvim rewrites it in place, so the stow symlink survives; commit the change after any `:Lazy update`, and use `:Lazy restore` to roll back to a committed state. `lazyvim.json` is tracked too (written in place by `lazyvim/util/json.lua`, so the symlink is likewise safe), but note that `extras` in it is empty — extras are declared in `lua/config/extras.lua`, and the rest is LazyVim's own state: a NEWS.md read marker plus schema-version fields it uses to decide whether to run a migration. Expect it to go dirty on LazyVim upgrades. Format Lua with stylua (`stylua.toml`: 2 spaces, 120 cols).
